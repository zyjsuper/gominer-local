...
func (sc *StratumClient) GetHeaderForWork() (target, header []byte, deprecationChannel chan bool, job interface{}, err error) {
	...
	for _, h := range sc.currentJob.MerkleBranch {
		...
		merkleRoot = DoubleSha256(m)
	}

	//Construct the header
	header = make([]byte, 0, 88)

	version, _ := hex.DecodeString(sc.currentJob.Version)
	header = append(header, Reverse(version)...)

	header = append(header, Swap32(sc.currentJob.PrevHash)...)

	header = append(header, merkleRoot...)

	header = append(header, Reverse(sc.currentJob.NTime)...)

	bits, _ := hex.DecodeString(sc.currentJob.NBits)
	header = append(header, Reverse(bits)...)

	header = append(header, []byte{0, 0, 0, 0}...)             // empty nonce
	header = append(header, []byte{0, 0, 0, 0, 0, 0, 0, 0}...) // empty target
...
func (sc *StratumClient) SubmitHeader(header []byte, job interface{}) (err error) {
	sj, _ := job.(stratumJob)
	nonce := hex.EncodeToString(Reverse(header[76:80]))
